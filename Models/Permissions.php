...
class Permissions extends Model {
	...
	public function setGroup($id, $id_company) {
		...
		if($sql->rowCount() > 0) {
			...
			//print_r($row['params']);exit;
			$params = $row['params'];
			// Pega o nome das permissões
	...
	public function add($name, $id_company) {
	 ...
	 public function addGroup($gname, $plist, $id_company) {
	 	...
	 	$sql = $this->db->prepare("INSERT INTO permission_groups SET gname = :gname, id_company = :id_company, params = :params");
	...
	public function editGroup($gname, $plist, $id, $id_company) {
	 	...
	 	$sql = $this->db->prepare("UPDATE permission_groups SET gname = :gname, id_company = :id_company, params = :params WHERE id = :id");
		$sql->bindValue(':gname', $gname);
		$sql->bindValue(':id_company', $id_company);
		$sql->bindValue(':params', $params);
		$sql->bindValue(':id', $id);
