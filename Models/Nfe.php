...
class Nfe extends Model {
	...
	public function emitirNFE($cNF, $destinatario, $prods, $fatinfo) {
		...
		$nfeTools = new \NFePHP\NFe\TollsNFe("nfe/files/config.json");
		...
		$mod = '55'; //modelo da NFe 55 NFe ou 65 NFCe=consumidor 
		$serie = '1'; //serie da NFe
		$nNF = $cNF; // numero da NFe // Salvar no BD
		...
		$IEST = $nfeTools->aConfig['iest']; // IE do Substituto Tributário
		...
		foreach($prods as $prod) {
		    ...
		    $EXTIPI = $prod['EXTIPI']; // Código de excessão do NCM-/qualncm.com.br
		...
		if($resp === true) {
			...
			// Gera o DANFE "necessário Certificado digital-SEFAZ.MT"
